	 ...
	 var id=myurl
	  $.ajax({
	   ...
	   success:function(result){
	   	if(result.success){ 
	   		var html=""
	   		var projectId=result["obj"].project.id;
			...
			var teacherId=result["obj"].project.tId;
			var nickname=result["obj"].project.nickname;
			var comments=result["obj"].project.comments;
    	...
    	'<div class="tab_bd" style="padding-bottom:20px;">'+
		    ...
		    '<input type="hidden" class="typeName" value="'+typeName+'">'+   
			'<input type="hidden" class="department" value="'+declareDepartment+'"> '+                                           
		    '<input type="hidden" class="projectTitle" value="'+pTitle+'">'+
		    '<input type="hidden" class="status" value="'+status+'">'+
		    '<input type="hidden" class="teacherId" value="'+teacherId+'">'+
    			...
    			'<div class="tab1_bd">'+
    				'<form action="" id="objectForm" method="post">'+
	    				...
	    				'<div class="tab_bd_con1">'+
	    					...
	    					'</div>'+
	    					'<div class="content_c1">'+
		    					'<div class="tab_bd_top clear endComment">'+
				    		      	'<div class="tab_bd_top_text1 ">意见评论</div>'+
				    		    	'<div class="tab_bd_con_list">'+
										'<textarea id="textarea5" rows="5" style="color: #222222;" class="hf_content"></textarea>'+
										'<button type="button" class="hf_button" onclick="huifu()">评论</button>'+
								'</div>'+
	    					'</div>'+
					    	'<div class="content_c1">'+
		    					'<div class="tab_bd_top clear endComment">'+
				    		      	'<div class="tab_bd_top_text1 ">专家最终审批意见</div>'+
				    		    	'<div class="tab_bd_con_list">'+
										'<textarea id="textarea5" rows="5" style="color: #222222;" class="comments">'+comments+'</textarea>'+
								'</div>'+
	    					'</div>'+
	    					'<div class="content_c1">'+
						    	'<div class="tab_bd_top clear">'+
    ...
    $.ajax({
	   ...
	   team+='<table border="1">'+
		...
		budget+='<tr>'+
	   	...
	   	budget+='</table>'
	   	$(".con_content_table3").html(budget); 
	   	//根据状态显示不同的按钮
	   	var btn="";
	   	if(status==1||status==2){
	   		//看似提交实则修改
	   		btn+='<button  type="button" class="reviewBtn" onclick="ReviewOk()" >审核通过</button>'+
              '<button  type="button" class="reviewBtn" onclick="ReviewNo()" style="background-color:#FA8072; ">审核不通过</button>';
	   	}
	   	$(".btn_span").html(btn);
	   		/*自动将status=1转为2*/
			if($(".status").val()==1){
			   $.ajax({
				   type:'post',
				   url:getRootPath+"/project/updateStatus",
				   data:{
					   "status":2,
					   "id":$(".projectId").val()
				   },
	 ...
	 function ReviewOk(){
		...
		if(comments==''||comments==null){
			mui.toast("专家最终审核意见不能为空");
		}else{
			 $.ajax({
				 type:'post',
				 url:getRootPath+'/project/review',
				 data:{
					 "comments":comments,
					 "status":3,
					 "id":$(".projectId").val()
				 ...
				 success:function(result){
					 if(result.success){
					 	mui.openWindow({
							id: "shenpi_detail",
							url: 'shenpi_detail.html?id='+projectId			
						});
						 mui.toast("审核结果已确认");
					 }else{
					     mui.toast(result.msg);
	 ...
	 function ReviewNo(){
	  ...
	  /*回复*/
	 function huifu(){
	 	var commentator=localStorage.getItem("nickname");
	 	var teacherId=localStorage.getItem("teacherId");
	 	var isProfessor=localStorage.getItem("isProfessor");
	 	var realName=localStorage.getItem("realName");
	    	var content=$(".hf_content").val();
	    	var toId=$(".teacherId").val();
	    	var toName=$(".nickname").val();
	    	var projectId=$(".projectId").val();
