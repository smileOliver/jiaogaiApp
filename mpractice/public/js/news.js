...
function data() {
	...
	mui.ajax({
	    ...
	    crossDomain: true,
	    type:'POST',
	    url:getRootPath+"/comment/selectByTeacherId",
	    data:{
	    	"page":counter,
	    	"pageSize":pageSize,
	    	"id":tid
	    ...
	    var html=""
	   	if(result.success){
   			...
   			$.each(result["obj"],function(i,value){
   				html+='<li class="mui-table-view-cell mui-media" id="'+value.projectId+'" >'+
							'<div class="mui-media-body">'+
								'<span style="padding-bottom:10px;">'+value.projectTitle+'</span>'+
								'<br>'+
	   		 ...
	   		 $(".mui-media").each(function() {  	
						...
						var jumpUrl ='comments.html?id='+ $(this).attr("id");	
						//console.log(jumpUrl);
							jump(jumpUrl);
						});
					});
...
function relTime(date){
	...
	var now=year+'年'+p(month)+"月"+p(date)+"日"+h+":"+m+":"+s;
	return now;
}
