 ...
 function data() {
	...
	mui.ajax({
	    ...
	    crossDomain: true,
	    type:'post',
	    url:getRootPath+"/assign/selectAssign",
	    jsonp: "jsonpCallback",
	    data:{
	    	"page":counter,
	    	"pageSize":pageSize,
	   	...
	   	if(result.success){
	   		if(result["obj"].length>0){
	   			$.each(result["obj"],function(i,value){	   				
	   				var id=value.id;
	   				html+='<li class="mui-table-view-cell mui-media" id="'+value.projectId+'" style="list-style:none;">'+
								'<div class="mui-media-body">'+
									'<span  style="color:black;">'+value.projectTitle+'</span>'+
									'<p>申请部门：'+value.declareDepartment+'</p>'+
									'<p>申请类型：'+value.typeName+'</p>'+
									'<p>发布时间：&nbsp;&nbsp;&nbsp;'+relTime(value.declareTime)+'</p>';
						if(value.status==1){
							html+='<p style="color:red;">状态：未查看</p>';
						}else if(value.status==2){
							html+='<p style="color:#FFFF00;">状态：已查看</p>';
	   		 ...
	   		 $('.mui-table-view').append(html);
	   		  //jQuery(html).insertBefore('#pullrefresh .mui-scroll .mui-table-view');
	   			$(".mui-media").each(function() {  	
						...
						var jumpUrl ='shenpi_detail.html?id='+ $(this).attr("id");				
							jump(jumpUrl);
						});
					});
	   		}else{
